...
Partida JugarPartida()
{
    ...
    while (aciertos_usuario < game.usada.barcos && aciertos_ordenador < game.usada.barcos)
    {
	    ...
	    acierto = ProcesarDisparo(game.disps.disparo[game.disps.nd].x,game.disps.disparo[game.disps.nd].y,barcos_ordenador,disparos_usuario,game.usada.filas,game.usada.columnas);
	    if (acierto == true)  
	    {
		   aciertos_usuario++;
		   cout << "BINGO!"<<endl;
	    ...
	    {   
	        ...
	        acierto = ProcesarDisparo(game.disps.disparo[game.disps.nd].x,game.disps.disparo[game.disps.nd].y,barcos_usuario,disparos_ordenador,game.usada.filas,game.usada.columnas);												      
	        
			system("cls");
            
	        VisualizarTableros(disparos_usuario,disparos_ordenador,game.usada.filas,game.usada.columnas);
		    
	        cout << "El ordenador ha disparado en fila " << game.disps.disparo[game.disps.nd].x+1 << ", columna " << game.disps.disparo[game.disps.nd].y+1 <<endl;
	        if (acierto == true)
	        {
		        aciertos_ordenador++;
		        cout << "BINGO!"<<endl;
